...
abstract class ChatRemoteDataSource {
  ...
  Future<void> sendTypingIndicator(int chatId);
  Future<void> stopTypingIndicator(int chatId);
...
class ChatRemoteDataSourceImpl implements ChatRemoteDataSource {
  ...
  Future<List<ChatModel>> searchChats(String query) async {
    final response = await dio.get('${ApiStringConstants.chatsUrl}/search', queryParameters: {'query': query});
    final List data = response.data['data'];
    return data.map((json) => ChatModel.fromJson(json)).toList();
  }
  ...
  Future<void> deleteChat(int chatId) async {
    await dio.delete('${ApiStringConstants.chatsUrl}/$chatId');
  }
  ...
  Future<void> sendTypingIndicator(int chatId) async {
    await dio.post(ApiStringConstants.typingUrl(chatId));
  }

  @override
  Future<void> stopTypingIndicator(int chatId) async {
    await dio.post(ApiStringConstants.stopTypingUrl(chatId));
  }
}
