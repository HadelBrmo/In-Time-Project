...
class CommentRemoteDataSourceImpl implements CommentRemoteDataSource {
  ...
  Future<List<CommentModel>> getCommentsForServing(int servingId) async {
    final response = await dio.get('/servings/$servingId/comments');
  ...
  Future<void> reactLike(int commentId) async {
    final response = await dio.post('/comments/$commentId/like');
    if (response.statusCode != 200) {
      throw ServerException();
    }
  }
  ...
  Future<void> reactDislike(int commentId) async {
    final response = await dio.post('/comments/$commentId/dislike');
    if (response.statusCode != 200) {
      throw ServerException();
    }
  }
